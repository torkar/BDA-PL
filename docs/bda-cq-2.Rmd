...
```{r m3, error=FALSE, message=FALSE, warning=FALSE, cache=TRUE, results='hide'}
...
We shouldn't expect $\beta$ (i.e., the parameter of a population-level effect) to vary extremely much, and we thus put $\mathcal{N}(0,0.5)$ as a prior on these effects (which is still fairly wide). After all, $\mathcal{N}(0,0.5)$, with four population-level effects, implies $(4 \times 0.5)^2 = \exp(2) \approx 54$, due to the $\log$ link emloyed in a Negative-Binomial.
...
M_priors <- brm(n_bugs ~ 1 + devs_log + max_commit_age_log + commits_log + insertions_s +
            ...
            (1 | project_id),
          family = negbinomial, data = toplas.data, prior = p, 
          sample_prior = "only")
```

```{r pripc, cache=TRUE,echo=FALSE, fig.margin=TRUE, warning=FALSE, message=FALSE, error=FALSE, fig.cap="Dashed vertical line is max value, while dark blue line is our data and light blue lines are draws from the priors"}
pp_check(M_priors) + 
...
Let's now conduct posterior predictive checks, i.e., see how well our model fits our data. But first we need to check the diagnostics.

## Diagnostics

The minimum effective sample size is above 10%. 

```{r}
table(neff_ratio(M)>0.1)
```

$\widehat{R}$ is consistently below 1.01,

```{r}
table(rhat(M) < 1.01)
```

and plotting the four chains for each parameter that was sampled we see the characteristic "fat hairy caterpillar" plots,
...
In the [final analysis](https://rpubs.com/torkar/572170) we'll look briefly at the differences the two models display, and how much the varying effects play a role in predictions. Finally, we'll also do simulations using one of the models, because that's where the rubber hits the road and all that we've done can comes to some use (except for trying to answer a question which really isn't interesting, i.e., generally speaking which language is the 'best').
...
```{r varsel, echo=TRUE, cahe=TRUE, results='hide'}
