...
\begin{aligned}
  ...
  \log(\lambda_i) & = \alpha_{\mathrm{LANG}[i]} \\
  \alpha_j & \sim \mathrm{Normal}(\bar{\alpha},\sigma_l) & \textrm{for LANG}=1,..,17\\
  \bar{\alpha} & \sim \mathrm{Normal}(0,5)\\
  \sigma_l & \sim \mathrm{Weibull}(2,1)\\
...
\begin{aligned}
  \mathrm{n\_bugs}_i & \sim \textrm{Negative-Binomial}(\lambda_i,\phi)\\
  \log(\lambda_i) & = \beta_1 \cdot \mathrm{devs\_log}_i + \beta_2 \cdot \mathrm{max\_commit\_age\_log}_i\\ 
                  & + \beta_3 \cdot \mathrm{commits\_log}_i + \beta_4 \cdot \mathrm{insertions\_log}_i\\
...
It might be prudent to set our own priors to see how the priors look like on the outcome scale.^[In this replication package we follow recommendations on priors. For an introduction please see (here)[https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations].] We'll set $\mathrm{Normal}(0,5)$ for our intercepts, $\mathrm{Weibull}(2,1)$ for our dispersion $\sigma$, and $\mathrm{Normal}(0,0.5)$ for each of our four $\beta$s. In math notation we now have this model,
...
\begin{aligned}
  \mathrm{n\_bugs}_i & \sim \textrm{Negative-Binomial}(\lambda_i,\phi) &\\
   \log(\lambda_i) & = \beta_1 \cdot \mathrm{devs\_log}_i + \beta_2 \cdot \mathrm{max\_commit\_age\_log}_i &\\ 
                  & + \beta_3 \cdot \mathrm{commits\_log}_i + \beta_4 \cdot \mathrm{insertions\_log}_i &\\
                  & + \alpha_{\mathrm{LANG}[i]} &\\
  \alpha_j & \sim \textrm{Normal}(\bar{\alpha},\sigma_l) & \textrm{for LANG}=1,..,17\\
  \bar{\alpha} & \sim \mathrm{Normal}(0,5) \\
  \sigma_l & \sim \mathrm{Weibull}{(2,1)} \\
...
\begin{aligned}
  \mathrm{n\_bugs}_i & \sim \textrm{Negative-Binomial}(\lambda_i,\phi)\\
  \log(\lambda_i) & = \alpha_{\mathrm{LANG}[i]} + \beta_{\mathrm{LANG}[i]} \cdot \mathrm{devs\_log}_i + \gamma_{\mathrm{LANG}[i]} \cdot \mathrm{max\_commit\_age\_log}_i \\
                  & + \delta_{\mathrm{LANG}[i]} \cdot \mathrm{commits\_log}_i + \epsilon_{\mathrm{LANG}[i]} \cdot \mathrm{insertions\_log}_i + \zeta_{\mathrm{PROJ}[i]}\\
...
\right)
\\
  \alpha,\ldots,\epsilon & \sim \mathrm{Normal}(0,0.5)\\
  \sigma_\alpha,\ldots,\sigma_\epsilon & \sim \mathrm{Weibull}(2,1)\\
  \mathbf{R} & \sim \mathrm{LKJ}(2)\\
  \zeta_j & \sim \mathrm{Normal(\bar{\alpha}, \sigma_p)} & \textrm{for PROJ} = 1,..,729\\
  \bar{\alpha} & \sim \mathrm{Normal(0,5)}\\
  \sigma_p & \sim \mathrm{Weibull(2,1)}\\
  \phi & \sim \textrm{Gamma}(0.01,0.01)
\end{aligned}
\end{equation}
...
Finally, we have also visually inspected the trace plots, but since this model estimates more than $837$ parameters we choose not to show it here.
...
# add +1 to everything so we can do log10 (it won't matter much anyways)
post <- post + 1

# sample 10 random projects and make long format
post %>% 
  sample(10) %>%
  mcmc_areas_ridges() + 
...
# In case one wants to compare deviations from 0 (intercept) draw samples from the posterior
# post <- posterior_samples(M)
# 
# post %>% 
...
nd_1stQ <- data.frame(language_id=seq(1:17),
                     project_id = NA,
                     devs_log=log(6),
                     max_commit_age_log=log(572),
                     commits_log=log(81),
                     insertions_log=log(7354))

nd_3rdQ <- data.frame(language_id=seq(1:17),
                     project_id = NA,
                     devs_log=log(36),
                     max_commit_age_log=log(1596),
                     commits_log=log(825),
                     insertions_log=log(149464))
...
PPC_1stQ <- posterior_predict(M, newdata = nd_1stQ)
PPC_3rdQ <- posterior_predict(M, newdata = nd_3rdQ)
PPC_min <- posterior_predict(M, newdata = nd_min)
PPC_max <- posterior_predict(M, newdata = nd_max)

colnames(PPC_1stQ) <- levels(fse.data$language)
colnames(PPC_3rdQ) <- levels(fse.data$language)
...
PPC_1stQ <- as.data.frame(PPC_1stQ) %>%
  gather(lang, bugs)

PPC_3rdQ <- as.data.frame(PPC_3rdQ) %>%
  gather(lang, bugs)
```
...
PPC_1stQ$bugs1 <- PPC_1stQ$bugs+1
PPC_3rdQ$bugs1 <- PPC_3rdQ$bugs+1
...
p1 <- ggplot(PPC_max, aes(x=reorder(lang, -bugs1), y=bugs1)) +
  ...
  ggtitle("Max (developers = 1383, max commit age = 16090, commits = 305361, insertions = 263641)") + 
...
p2 <- ggplot(PPC_median, aes(x=reorder(lang, -bugs1), y=bugs1)) +
  geom_violin() +
  theme(axis.text.x = element_text(angle=45, hjust=1)) + xlab("") + ylab("") + 
  ggtitle("Median (developers = 15, max commit age = 1029, commits = 248, insertions = 263641)") + 
...
p1stQ <- ggplot(PPC_1stQ, aes(x=reorder(lang, -bugs1), y=bugs1)) +
  geom_violin() +
  theme(axis.text.x = element_text(angle=45, hjust=1)) + xlab("") + ylab("") + 
  ggtitle("25th percentile (developers = 346, max commit age = 4023, commits = 76340, insertions = 6764171)")+ 
  scale_y_continuous(minor_breaks = NULL, 
                     trans="log10") + 
  geom_hline(yintercept=median(PPC_1stQ$bugs1), size=0.2)

p3rdQ <- ggplot(PPC_3rdQ, aes(x=reorder(lang, -bugs1), y=bugs1)) +
  geom_violin() +
  theme(axis.text.x = element_text(angle=45, hjust=1)) + xlab("") + ylab("") + 
  ggtitle("75th percentile (developers = 1038, max commit age = 12068, commits = 229021, insertions = 20292514)")+ 
  scale_y_continuous(minor_breaks = NULL, 
                     trans="log10") + 
  geom_hline(yintercept=median(PPC_3rdQ$bugs1), size=0.2)

p1/p3rdQ/p2/p1stQ/p3
```
