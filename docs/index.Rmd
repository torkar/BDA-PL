...
In the plot above we've $\log10()$ transformed the $x$-axis so we can see what's actually happening. In short, the priors are all over the place (and a number of them are $\infty$). The maximum number of bugs in our dataset is hardly $\gt10^{9}$.
