...
# Set cmdstanr as backend for brms if you haveit
...
pp_check(m0p, nsamples = 100) + scale_x_log10()
```


### Posterior predictive checks

Running the above model with default priors and settings (in later examples we will set our own priors),^[Here and in the following examples we've set a seed in order for the results to be reproducible. During the development one should never set a seed---the stochastic elements of these algorithms are key to their reliability.]

```{r, message=FALSE, warning=FALSE, error=FALSE, message='hide'}
...
pp_check(m0, nsamples = 100) + scale_x_log10()
```

As we can see, the model struggles and we don't have a good fit. We'll add the other variables at our disposal and see if we can improve things. But first, let's have a look at the diagnostics.
...
(p <- get_prior(n_bugs ~ 1 + devs_log + max_commit_age_log + commits_log + 
                ...
                data = fse.data, family = negbinomial))
```
...
m1 <- brm(n_bugs ~ 1 + devs_log + max_commit_age_log + commits_log +
                 insertions_log + (1 | language_id),
          family = negbinomial(), prior = p, data=fse.data, 
          chains = 1, iter = 1000, 
...
pp_check(m1, nsamples = 100) + scale_x_log10()
```
...
m2 <- brm(n_bugs ~ 1 + devs_log + max_commit_age_log + commits_log + insertions_log +
            (1 + devs_log + max_commit_age_log + commits_log + insertions_log | language_id) +
            (1 | project_id),
          family = negbinomial(), prior = p, 
          data=fse.data, chains = 1, iter = 1000, 
          sample_prior = "only", refresh = 0)
```

```{r m2-post, warning=FALSE, message=FALSE}
pp_check(m2, nsamples = 100) + scale_x_log10()
```
...
```{r}
hist(table(fse.data$project_id)+0.001, main="", xlab="Number of rows", xaxt="n", 
     labels = TRUE, ylim = c(0,500))
...
tibble(x = seq(from = 0, to = 3, by = .1)) %>% 
  ...
  annotate("text", x = 0.5, y = 20, label = "Posterior", size=6)
```
...
post %>% 
  ...
  xlab("Number of bugs") + scale_x_continuous(trans="log10")
