...
The prior predictive checks (where we use default priors for now), show that the priors allow large (even absurd) values (the $x$-axis has been `log10()` transformed). The dashed line indicates the maximum number of bugs in our empirical data.
...
The $\mathrm{Gamma}$ prior on $\phi$ allows only positive real numbers, as does the $\mathrm{Weibull}(2,1)$ for our varying intercepts. Additionally, having $\mathrm{Normal}(0,0.05)$ on our $\beta$s might seem tight, but the additive terms imply a variance of $(0.5*4)^2=4$ (and then we also use a log link function! Let's see what the prior predictive checks will show. 
...
In the plot above we've $\log10()$ transformed the $x$-axis so we can see what's actually happening. In short, the priors are all over the place (and a number of them are $\infty$). The maximum number of bugs in our dataset is depicted by the dashed line.
...
We see that the model has a much better fit to the empirical data. This is an indication that adding the four population-level parameters makes a difference.
